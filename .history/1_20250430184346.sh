...
while true; do
    ...
    MENU_ITEMS=()
    # local index=1 # index 变量在此处未使用，可以移除
    for acc in "${ACCOUNTS_LIST[@]}"; do
        MENU_ITEMS+=("$acc" "配置 $acc")
        # ((index++)) # index 变量在此处未使用，可以移除
    done
    MENU_ITEMS+=("ADD" "添加新账号")

    # 计算实际的菜单项数量 (账号数 + 添加项)
    local num_entries=$(( ${#ACCOUNTS_LIST[@]} + 1 ))

    # 设置列表的期望显示高度 (例如，最多显示 8 行，但不少于实际项数，除非项数太多)
    local list_height=$num_entries
    # 如果希望列表最多显示 8 行，即使项目更多也启用滚动
    [[ $list_height -gt 8 ]] && list_height=8
    # 确保列表高度至少为 1
    [[ $list_height -lt 1 ]] && list_height=1

    # 计算对话框的总高度：列表高度 + 额外空间 (标题、提示、边框等，大约需要 6-7 行)
    local menu_height=$(( list_height + 7 ))
    # 可以设置一个最大总高度，防止对话框过大
    # [[ $menu_height -gt 20 ]] && menu_height=20
    # 确保总高度至少能容纳基本元素 (例如，至少 8 行高)
    ...
    CHOICE=$(dialog --clear --backtitle "QQ账号管理" \
                    ...
                    "$menu_height" 55 "$list_height" \ # 使用计算出的 menu_height 和 list_height
                    "${MENU_ITEMS[@]}" \
                    2>&1 >/dev/tty)
