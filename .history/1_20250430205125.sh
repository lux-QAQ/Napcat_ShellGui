...
configure_http_server() {
    ...
    while true; do
        ...
        if [[ "$mode" == "modify" ]]; then
            ...
            # 使用 |= 确保即使 network 或 httpServers 为 null 也能正确处理
            jq_script='
                (.network //= {}) |
                (.network.httpServers //= []) |
        ...
        if jq --argjson new_obj "$new_server_obj" "$jq_script" "$config_file" > "$temp_file" 2> "$jq_stderr_file"; then
            ...
            if [[ -s "$temp_file" ]]; then
                 ...
                 if mv "$temp_file" "$config_file"; then
                     ...
                     rm -f "$jq_stderr_file" # 成功后删除 stderr 文件
                     trap - EXIT # 成功完成，取消 EXIT trap
                     return 0 # 成功，退出函数
                 else
                     dialog --msgbox "错误：无法更新配置文件 '$config_file'。权限问题？" 8 60
                     rm -f "$jq_stderr_file" # 出错也删除 stderr 文件
                     # rm -f "$temp_file" # 保留临时文件供调试？
                     return 1
                 fi
            else
                 local jq_error=$(<"$jq_stderr_file") # 读取 stderr 文件内容
                 dialog --msgbox "错误：jq 处理后生成了空文件。请检查 jq 脚本和输入。\n\nJQ 输出(可能为空):\n$(cat "$temp_file")\n\nJQ 错误:\n$jq_error" 15 70
