...
# 检查 dialog --colors --colors 是否安装
if ! command -v dialog --colors --colors &> /dev/null; then
    echo "错误: 需要 'dialog --colors --colors' 命令，请先安装。" >&2
    echo "例如: sudo apt update && sudo apt install dialog --colors --colors" >&2
    exit 1
fi
...
RESET='\Zn'      # 重置为正常
BOLD='\Zb'       # 切换粗体 (效果取决于终端和 dialog --colors --colors 版本)
...
configure_http_server() {
    ...
    if [[ ! -f "$config_file" ]] || [[ ! -r "$config_file" ]]; then
        dialog --colors --colors --msgbox "错误：无法读取配置文件 '$config_file'。" 8 60
        return 1
    fi
    ...
    while true; do
        ...
        # 1. 输入框部分
        exec 3>&1
        local form_values=$(dialog --colors --colors --clear --backtitle "HTTP 服务端配置" \
        ...
        if [[ $form_exit_status -ne 0 ]]; then
            dialog --colors --colors --msgbox "操作已取消。" 6 40
            return 1
        fi
        ...
        # 2. 消息格式选择
        exec 3>&1
        local msg_format_choice=$(dialog --colors --colors --clear --backtitle "HTTP 服务端配置" \
        ...
        # 3. 开关选择
        exec 3>&1
        local checklist_choices=$(dialog --colors --colors --clear --backtitle "HTTP 服务端配置" \
        ...
        if [[ ${#errors[@]} -gt 0 ]]; then
            ...
            dialog --colors --colors --msgbox "$error_msg" 15 60
            # 保留用户输入以便下次显示
            current_name="$name"
            current_host="$host"
            current_port="$port"
        ...
        if jq --argjson new_obj "$new_server_obj" "$jq_script" "$config_file" > "$temp_file" 2> "$jq_stderr_file"; then
            ...
            if [[ -s "$temp_file" ]]; then
                 ...
                 if mv "$temp_file" "$config_file"; then
                     dialog --colors --colors --msgbox "${FG_GREEN}HTTP 服务端配置已成功 $title_prefix！${RESET}" 8 50
                     rm -f "$jq_stderr_file"
                     trap - EXIT
                     return 0
                 else
                     dialog --colors --colors --msgbox "${FG_RED}错误：${RESET}无法更新配置文件 '$config_file'。权限问题？" 8 60
                     rm -f "$jq_stderr_file"
                     return 1
                 fi
            else
                 ...
                 # 错误消息用红色
                 dialog --colors --colors --msgbox "${FG_RED}错误：${RESET}jq 处理后生成了空文件。请检查 jq 脚本和输入。\n\nJQ 输出(可能为空):\n$(cat "$temp_file")\n\n${FG_RED}JQ 错误:${RESET}\n$jq_error" 15 70
                 rm -f "$jq_stderr_file"
                 return 1
            fi
        else
            ...
            rm -f "$jq_stderr_file"
            # 错误消息用红色
            dialog --colors --colors --msgbox "${FG_RED}错误：${RESET}使用 jq 更新 JSON 时出错。\n\n${FG_RED}JQ 错误:${RESET}\n$jq_error\n\n请检查 JSON 文件格式或 jq 命令。" 15 70
            return 1
        fi
...
get_qq_accounts() {
    ...
    if [[ ! -d "$CONFIG_DIR" ]]; then
        dialog --colors --colors --msgbox "错误：配置目录 '$CONFIG_DIR' 不存在。" 8 50
        return 1
    fi
...
add_account() {
    while true; do
        NEW_QQ=$(dialog --colors --colors --clear --backtitle "添加账号" \
                        --title "输入新QQ账号" \
        ...
        if [[ $exit_status -ne 0 ]]; then
            dialog --colors --colors --msgbox "添加操作已取消。" 6 40
            return 1 # 表示取消
        fi
        ...
        if [[ "$NEW_QQ" =~ ^[0-9]{4,}$ ]]; then
            ...
            if [[ -f "$new_napcat_file" || -f "$new_onebot_file" ]]; then
                 dialog --colors --colors --yesno "账号 $NEW_QQ 的配置文件已存在，是否覆盖？" 8 50 2>&1 >/dev/tty
                 if [[ $? -ne 0 ]]; then # 用户选择 "否"
                     continue # 重新提示输入
                 fi
            fi
            ...
            if [[ ! -f "$BASE_NAPCAT_CONFIG" ]]; then
                dialog --colors --colors --msgbox "错误：基础配置文件 '$BASE_NAPCAT_CONFIG' 不存在。" 8 60
                return 2 # 表示基础文件缺失
            fi
             if [[ ! -f "$BASE_ONEBOT_CONFIG" ]]; then
                dialog --colors --colors --msgbox "错误：基础配置文件 '$BASE_ONEBOT_CONFIG' 不存在。" 8 60
                return 2 # 表示基础文件缺失
            fi
            ...
            if [[ $? -eq 0 ]]; then
                dialog --colors --colors --msgbox "账号 $NEW_QQ 添加成功！\nNapcat 配置: $new_napcat_file\nOneBot 配置: $new_onebot_file" 10 70
                return 0 # 表示成功
            else
                dialog --colors --colors --msgbox "错误：复制文件时出错，请检查权限或磁盘空间。" 8 60
                return 3 # 表示复制失败
            fi
        else
            dialog --colors --colors --msgbox "输入无效！请输入至少4位数字。" 6 40
            # 循环继续，要求重新输入
        fi
    done
}
...
show_service_menu() {
    local qq_account=$1
    while true; do
        SERVICE_CHOICE=$(dialog --colors --colors --clear --backtitle "网络服务配置" \
        ...
        case "$SERVICE_CHOICE" in
            1)
                ...
                ;;
            2)
                dialog --colors --colors --msgbox "将为账号 $qq_account 配置 HTTP 客户端 (反向http) - 功能待实现" 8 60
                # configure_reverse_http_client "$qq_account"
                ;;
            3)
                dialog --colors --colors --msgbox "将为账号 $qq_account 配置 WebSocket 服务端 (正向ws) - 功能待实现" 8 60
                # configure_ws_server "$qq_account"
                ;;
            4)
                dialog --colors --colors --msgbox "将为账号 $qq_account 配置 WebSocket 客户端 (反向ws) - 功能待实现" 8 60
                # configure_reverse_ws_client "$qq_account"
                ;;
            *)
                dialog --colors --colors --msgbox "无效的选择: $SERVICE_CHOICE" 6 40
                ;;
        esac
...
while true; do
    ...
    if [[ $? -ne 0 ]]; then # 如果 get_qq_accounts 失败 (例如目录不存在)
        dialog --colors --colors --msgbox "错误：无法获取账号列表，请检查配置目录 '$CONFIG_DIR'。" 8 60
        exit 1
    fi
    ...
    CHOICE=$(dialog --colors --colors --clear --backtitle "QQ账号管理" \
                    --title "选择QQ账号" \
        ...
        *)
            ...
            if $is_valid_account; then
                ...
                # 从菜单2返回后，循环继续，回到账号选择菜单
                 ...
                 # 这通常不应该发生，因为dialog --colors --colors只返回菜单中的有效项
                 dialog --colors --colors --msgbox "出现意外错误，无效的选择: $CHOICE" 6 40
            fi
            ;;
    esac
done
