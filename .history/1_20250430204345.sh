...
configure_http_server() {
    ...
    local title_prefix="新建"
    [[ "$mode" == "modify" ]] && title_prefix="修改"
    ...
    while true; do
        ...
        if [[ "$mode" == "modify" ]]; then
            ...
            jq_script='.network.httpServers[0] = ($new_obj | fromjson)'
        else
            # 新建模式：确保 network 和 httpServers 存在，然后添加
            jq_script='
                (.network //= {}) |
                (.network.httpServers //= []) |
                .network.httpServers += [($new_obj | fromjson)]
            '
        fi

        if jq --argjson new_obj "$new_server_obj" "$jq_script" "$config_file" > "$temp_file"; then
            # 检查写入临时文件是否成功且内容不为空
            if [[ -s "$temp_file" ]]; then
                 # 备份原始文件 (可选但推荐)
                 ...
                 if mv "$temp_file" "$config_file"; then
                     dialog --msgbox "HTTP 服务端配置已成功 $title_prefix！" 8 50
                     rm -f "$temp_file" # 确保成功移动后删除临时文件
                     trap - EXIT # 取消 EXIT trap
                     return 0 # 成功，退出函数
                 else
                     dialog --msgbox "错误：无法更新配置文件 '$config_file'。权限问题？" 8 60
                     # rm -f "$temp_file" # 保留临时文件供调试？
                     return 1
                 fi
            else
                 dialog --msgbox "错误：jq 处理后生成了空文件。请检查 jq 脚本和输入。" 8 70
                 return 1
            fi
        else
            dialog --msgbox "错误：使用 jq 更新 JSON 时出错。请检查 JSON 文件格式或 jq 命令。" 10 70
            cat "$temp_file" > debug_error.txt # 显示 jq 的错误输出可能有用
            return 1
        fi
