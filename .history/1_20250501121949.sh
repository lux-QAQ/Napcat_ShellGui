    ...
    while true; do
        ...
        case "$SERVICE_CHOICE" in
            ...
            HELP)
                # 定义帮助信息，使用颜色代码
                local help_text=""
                help_text+="${FG_GREEN}HTTP 服务端${RESET}\n"
                help_text+="  ${FG_CYAN}NapCat${RESET} 作为 HTTP 服务端，监听并接受 ${FG_YELLOW}外部${RESET} 发起的请求，处理后返回响应 (${FG_MAGENTA}单向${RESET}：${FG_YELLOW}外部${RESET}→${FG_CYAN}NapCat${RESET})。\n\n"
                help_text+="${FG_GREEN}HTTP 客户端${RESET}\n"
                help_text+="  ${FG_CYAN}NapCat${RESET} 作为 HTTP 客户端，主动向 ${FG_YELLOW}外部${RESET} 接口发起请求，将内部事件推送给插件或应用框架 (${FG_MAGENTA}单向${RESET}：${FG_CYAN}NapCat${RESET}→${FG_YELLOW}外部${RESET})。\n\n"
                help_text+="${FG_GREEN}WebSocket 服务端${RESET}\n"
                help_text+="  ${FG_CYAN}NapCat${RESET} 作为 WebSocket 服务端，与 ${FG_BLUE}客户端${RESET} 保持 ${FG_MAGENTA}双向${RESET} 长连接，既能被动接收 ${FG_BLUE}客户端${RESET} 消息，也能主动推送事件 (${FG_MAGENTA}双向${RESET}：${FG_CYAN}NapCat${RESET}↔${FG_BLUE}客户端${RESET})。\n\n"
                help_text+="${FG_GREEN}WebSocket 客户端${RESET}\n"
                help_text+="  ${FG_CYAN}NapCat${RESET} 作为 WebSocket 客户端，主动向 ${FG_YELLOW}外部${RESET} WebSocket ${FG_BLUE}服务端${RESET} 建立 ${FG_MAGENTA}双向${RESET} 长连接，既能主动发送事件，也能响应 ${FG_BLUE}服务端${RESET} 请求 (${FG_MAGENTA}双向${RESET}：${FG_CYAN}NapCat${RESET}↔${FG_BLUE}服务端${RESET})。\n\n"
                help_text+="详细配置请参考：${FG_WHITE}https://napcat.napneko.icu/config/basic${RESET}"

                # 显示帮助信息
                dialog --colors --cr-wrap --title "如何选择网络服务？" --msgbox "$help_text" 22 75
