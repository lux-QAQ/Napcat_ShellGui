...
BASE_ONEBOT_CONFIG="$CONFIG_DIR/onebot11.json"
LOG_FILE="/tmp/qqbot_gui_debug_$(date +%s).log" # 添加日志文件定义
...
configure_http_server() {
    ...
    while true; do
        # --- 显示表单 ---
        # 1. 输入框部分
        # 创建一个临时文件来存储 dialog 的输出 (stderr)
        local temp_dialog_output=$(mktemp) || { echo "无法创建临时文件"; exit 1; }
        # 确保所有临时文件都被 trap 处理
        trap 'rm -f "$temp_file" "$temp_dialog_output"' EXIT

        # 直接运行 dialog，将其 stderr 重定向到临时文件
        dialog --colors --clear --backtitle "HTTP 服务端配置" \
            ...
            2> "$temp_dialog_output" # 将 stderr 重定向到临时文件

        # *立即* 捕获 dialog 的退出状态
        local form_exit_status=$?
        clear

        echo "$(date '+%Y-%m-%d %H:%M:%S') - Cancelled form dialog in configure_http_server. Exit status: $form_exit_status. Returning..." >> 1.txt
        if [[ $form_exit_status -ne 0 ]]; then
            echo "$(date '+%Y-%m-%d %H:%M:%S') - Cancelled form dialog in configure_http_server. Exit status: $form_exit_status. Returning..." >> "$LOG_FILE"
        ...
        local checklist_choices=$(dialog --colors --clear --backtitle "HTTP 服务端配置" \
            ...
            "enable"    "启用服务（${FG_RED}需手动确认${RESET}）"      "$current_enable" \
...
configure_ws_server() {
    ...
    while true; do
        ...
        # 1. 输入框部分
        exec 3>&1
        local form_values=$(dialog --colors --clear --backtitle "WebSocket 服务端配置" \
...
configure_reverse_ws_client() {
    ...
    while true; do
        ...
        # 1. 输入框部分
        exec 3>&1
        local form_values=$(dialog --colors --clear --backtitle "WebSocket 客户端配置" \
